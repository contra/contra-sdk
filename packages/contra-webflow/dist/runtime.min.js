"use strict";var ContraWebflow=(()=>{var E=Object.defineProperty;var x=Object.getOwnPropertyDescriptor;var L=Object.getOwnPropertyNames;var A=Object.prototype.hasOwnProperty;var T=(o,t)=>{for(var e in t)E(o,e,{get:t[e],enumerable:!0})},S=(o,t,e,i)=>{if(t&&typeof t=="object"||typeof t=="function")for(let s of L(t))!A.call(o,s)&&s!==e&&E(o,s,{get:()=>t[s],enumerable:!(i=x(t,s))||i.enumerable});return o};var R=o=>S(E({},"__esModule",{value:!0}),o);var M={};T(M,{ContraWebflowRuntime:()=>g,default:()=>g});var b=class f{constructor(t){this.cache=new Map,this.pendingRequests=new Map,this.config={baseUrl:"https://contra.com",timeout:1e4,debug:!1,...t},this.config.debug&&console.log("[ContraClient] Initialized with config:",this.config)}async fetch(t,e={},i=3){let s=`${this.config.baseUrl}${t}`,a=new AbortController,r=setTimeout(()=>a.abort(),this.config.timeout),n={...e,signal:a.signal,headers:{Accept:"application/json","X-API-key":this.config.apiKey,...e.headers}};try{this.config.debug&&console.log(`[ContraClient] Fetching: ${s}`,n);let l=await fetch(s,n);if(clearTimeout(r),!l.ok){let h;try{h=await l.json()}catch{h={code:`HTTP_${l.status}`,message:l.statusText||"Unknown error"}}if((l.status>=500||l.status===429)&&i>0){let m=Math.pow(2,3-i)*1e3;return await new Promise($=>setTimeout($,m)),this.fetch(t,e,i-1)}throw new p(h.message,h.code,l.status)}let u=await l.json();return this.config.debug&&console.log("[ContraClient] Response:",u),u}catch(l){if(clearTimeout(r),l instanceof p)throw l;if(l instanceof Error&&l.name==="AbortError")throw new p(`Request timeout after ${this.config.timeout}ms`,"TIMEOUT");if(i>0){let u=Math.pow(2,3-i)*1e3;return await new Promise(h=>setTimeout(h,u)),this.fetch(t,e,i-1)}throw new p(l instanceof Error?l.message:"Unknown error","NETWORK_ERROR")}}async fetchWithCache(t,e,i,s){let a=this.cache.get(t);if(a&&Date.now()-a.timestamp<a.ttl)return this.config.debug&&console.log(`[ContraClient] Cache hit: ${t}`),a.data;let r=`${e}${JSON.stringify(s)}`;if(this.pendingRequests.has(r))return this.config.debug&&console.log(`[ContraClient] Request deduplication: ${r}`),this.pendingRequests.get(r);let n=this.fetch(e,s);this.pendingRequests.set(r,n);try{let l=await n;return this.cache.set(t,{data:l,timestamp:Date.now(),ttl:i}),l}finally{this.pendingRequests.delete(r)}}buildQueryString(t){let e=new URLSearchParams;Object.entries(t).forEach(([s,a])=>{a!=null&&a!==""&&(Array.isArray(a)?e.set(s,a.join(",")):e.set(s,String(a)))});let i=e.toString();return i?`?${i}`:""}async getProgram(t){let e=`program:${t}`,i=`/public-api/programs/${t}`;return(await this.fetchWithCache(e,i,f.CACHE_TTL.program)).data}async listExperts(t,e={}){let i=this.buildQueryString(e),s=`experts:${t}:${JSON.stringify(e)}`,a=`/public-api/programs/${t}/experts${i}`;return this.fetchWithCache(s,a,f.CACHE_TTL.experts)}async searchExperts(t,e,i={}){let s=await this.listExperts(t,i);if(e.trim()){let a=e.toLowerCase();s.data=s.data.filter(r=>r.name.toLowerCase().includes(a)||r.oneLiner.toLowerCase().includes(a)||r.skillTags.some(n=>n.toLowerCase().includes(a)))}return s}async getFilterOptions(t){let e=`filters:${t}`,i=`/public-api/programs/${t}/filters`;return this.fetchWithCache(e,i,f.CACHE_TTL.filters)}clearCache(t){if(t)for(let e of this.cache.keys())e.includes(t)&&this.cache.delete(e);else this.cache.clear();this.config.debug&&console.log(`[ContraClient] Cache cleared${t?` (pattern: ${t})`:""}`)}getCacheStats(){let t=Date.now(),e=Array.from(this.cache.entries()).map(([i,s])=>({key:i,age:t-s.timestamp,ttl:s.ttl}));return{size:this.cache.size,entries:e}}};b.CACHE_TTL={experts:5*60*1e3,expert:10*60*1e3,program:30*60*1e3,filters:60*60*1e3};var C=b,p=class extends Error{constructor(o,t,e){super(o),this.code=t,this.status=e,this.name="ContraAPIError"}},y={formatRate(o){return o?`$${o}/hr`:"Rate on request"},renderStars(o){let t=Math.floor(o),e=o%1>=.5,i=5-t-(e?1:0),s="";for(let a=0;a<t;a++)s+=`<svg class="star star-full" width="16" height="16" viewBox="0 0 24 24" fill="#FFD700">
